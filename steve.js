...
const remove_selected_from_lenght = () => {
  ...
  for (let i = 0; i < get_dom_list()[0].children.length; i++) {
    get_dom_list()[0].children[i].addEventListener("click", (e) => {
      ...
      for (let n = 0; n < get_dom_list()[1].children.length; n++) {
        if (!get_dom_list()[1].children[n].classList.value.includes("custom")) {
          get_dom_list()[1].children[n].classList.remove('selected');
		   get_dom_list()[1].children[n].setAttribute('aria-checked', 'false');
        }
      }
      for (let n = 0; n < get_dom_list()[2].children.length; n++) {
        if (get_dom_list()[2].children[n].classList.contains("selected")) {
           get_dom_list()[2].children[n].classList.remove('selected');
		   get_dom_list()[2].children[n].setAttribute('aria-checked', 'false');
        }
      }
    });
  }
